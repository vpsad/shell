...
clear
echo -e "\033[31m若在使用中发现问题，请及时反馈！QQ群：613980114 \033[0m"
sleep 3s

if [ ! -d /www/server/panel/BTPanel ];then
	echo "============================================="
	echo "VPS博客提醒您：www.vpsad.cn"
	echo "错误, 5.x不可以使用此命令升级!"
	echo "5.9平滑升级到6.0的命令：curl http://download.bt.cn/ltd/install/update_to_6.sh|bash"
	exit 0;
fi
...
downloads_Url=https://www.05bt.com
setup_path=/www
version=$(curl -Ss --connect-timeout 5 -m 2 https://www.05bt.com/api/panel/get_version)
if [ "$version" = '' ];then
	version='7.6.20'
fi
